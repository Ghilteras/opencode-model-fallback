export interface FallbackPluginConfig {
	...
	/** Chain of models tried by the experimental.provider.small_model hook.
	 *  Format: "provider/model". Provider cooldowns are shared with the
	 *  fallback chain (smallModelProviderCooldowns). */
	small_model_chain?: string[]
	/** When true, a session that swapped to a fallback never recovers to the
	 *  original model for its lifetime (no recovery bounce = no cache wipe). */
	sticky_fallback?: boolean
}
...
export interface FallbackState {
	...
	/** Session pinned to the fallback for its lifetime (non-transient error
	 *  swap). Blocks recovery to the original model. */
