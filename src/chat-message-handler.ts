...
export function createChatMessageHandler(deps: HookDeps, helpers: AutoRetryHelpers) {
	...
	return async (input: ChatMessageInput, output: ChatMessageOutput) => {
		...
		// 2026-08-14: per-session stickiness. A session that swapped due to a
		// NON-TRANSIENT error (state.sessionPinned) stays on the fallback for its
		// lifetime — no recovery bounce = no cache wipe. Sessions that transiently
		// bounced (not pinned) recover to primary after cooldown.
