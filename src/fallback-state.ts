...
import { smallModelProviderCooldowns } from "./constants"
...
function applyFallbackPlan(
	...
	// Provider-level cooldown shared with the small_model chain: a provider
	// that failed a fallback swap is skipped by the small_model chain too.
	const failedProvider = String(plan.failedModel).split("/")[0]
	if (failedProvider) smallModelProviderCooldowns.set(failedProvider, Date.now())
	state.attemptCount++
