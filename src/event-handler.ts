...
export function createEventHandler(deps: HookDeps, helpers: AutoRetryHelpers) {
	...
	const handleSessionError = async (props: Record<string, unknown> | undefined) => {
		...
		try {
			...
			// 2026-08-14: fallback ONLY on non-transient errors. Transient (429/5xx)
			// retry NATIVELY on the current provider (preserves cache). A non-transient
			// error (e.g. insufficient_balance/402) is when we swap + pin the session.
			...
			if (action === "stay-pinned") {
				// Transient on an already-swapped (pinned) session: native retry on the
				// fallback provider, NO chain advance, NO unpin (primary is dead for this
				// session). Pin untouched → recovery gate keeps it on the fallback.
			...
			// 2026-08-14: pin the session to the fallback for its lifetime, but ONLY on
			// non-transient paths (swap-and-pin / advance). Transient paths returned
			// above, so this can never re-pin a session after a transient error. The
			// session stays on the fallback (recovery gate blocked) — no bounce back to
			// a dead primary = no cache wipe.
